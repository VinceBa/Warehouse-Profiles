...
class Profile:
   ...
   def execute(self, obj):
        ...
        if L:                                   
            ...
            if B1Y or B2Y or B1X or B2X:       # make the bevels:        
                ...
                if obj.WidthCentered == True:
                    p1 = v(w,h,     L-Z2Y/2)
                    p2 = v(w+W,h,   L+Z2Y/2)
                    p3 = v(w+W,h,   L+Z2Y/2+ZC)
                    p4 = v(w,h,     L+Z2Y/2+ZC)
                    dZ = L+Z2Y/2+ZC
                ProfileExt = ProfileTemp.fuse(p.extrude(v(0,0,dZ)))
                ...
                if obj.WidthCentered == True:
                    if B1Y>0:
                        ...
                        p4 = v(w,h,     Z1Y/2-ZC)
                    dZ = -ZC-Z1Y/2
                    if B1Y>0: p.translate(v(0,0,Z1Y))
                ProfileExt = ProfileTemp.fuse(p.extrude(v(0,0,dZ)))    
