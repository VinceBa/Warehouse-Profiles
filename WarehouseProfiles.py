...
#               and More profiles!
#               icons
#               Make profiles on the fly
#               Separate Rotate bevel 
#               
...
class Profile:
   ...
   def execute(self, obj):
        ...
        if W == 0 : W = H
        k =  10 * max (H/W,W/H)  # coeff pour depasser l'objet (rotation bevel)
        ...
        if L:                                   
            ...
            if B1 or B2:                           # couper les extrémités
                Z1 = W*math.tan(math.pi*B1/180)
                Z2 = W*math.tan(math.pi*B2/180)
                ZA = W*math.tan(math.pi*A/180)
                ZC1 = k*(abs(Z1)+abs(ZA))
                ZC2 = k*(abs(Z2)+abs(ZA))
                ...
                if B1 and not B2:
                    if obj.WidthCentered == False:          
                        ...
                        p4 = v(W+w,h,-ZC1)
                        p5 = v(w,h,-ZC1)
                        ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,-ZC1/5)))
                        ProfileCut = ProfileExt.cut(self.SubtractiveCoin(p1,p3,p4,p5,H,A,k))
                    ...
                    if obj.WidthCentered == True:
                        if B1>0:
                            ...
                            p4 = v(w,h,-Z1/2-ZC1)
                        ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,-ZC1/5)))    
                        ProfileCut = ProfileExt.cut(self.SubtractiveCoin(p1,p2,p3,p4,H,A,k))
                        obj.Shape  = ProfileCut.removeSplitter()
                ...
                if B2 and not B1: 
                    if obj.WidthCentered == False: 
                        ...
                        p4 = v(W+w,h,L+ZC2)
                        p5 = v(w,h,L+ZC2)
                        ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,L+ZC2/5)))
                        ProfileCut = ProfileExt.cut(self.SubtractiveCoin(p1,p3,p4,p5,H,A,k))
                        obj.Shape  = ProfileCut.removeSplitter()    
                    ...
                    if obj.WidthCentered == True:
                        if B2>0:
                            ...
                            p3 = v(w+W,h,L-Z2/2+ZC2)
                            p4 = v(w,h,L-Z2/2+ZC2)
                        ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,L+abs(Z2+ZC2)/2)))
                        ProfileCut = ProfileExt.cut(self.SubtractiveCoin(p1,p2,p3,p4,H,A,k))
                        obj.Shape  = ProfileCut.removeSplitter()
                ...
                if B1 and B2 :                  
                    if obj.WidthCentered == False: 
                        ...
                        p5 = v(w,h,L+ZC2)
                        ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,L+ZC2/5)))
                        ProfileCut1 = ProfileExt.cut(self.SubtractiveCoin(p1,p3,p4,p5,H,A,k))
                        p1 = v(w,h,0)
                        p3 = v(W+w,h,Z1)
                        p4 = v(W+w,h,-ZC1)
                        p5 = v(w,h,-ZC1)
                        ProfileExt = ProfileCut1.fuse(p.extrude(v(0,0,-ZC1/5)))
                        ProfileCut2 = ProfileExt.cut(self.SubtractiveCoin(p1,p3,p4,p5,H,A,k))
                        obj.Shape  = ProfileCut2.removeSplitter()
                    ...
                    if obj.WidthCentered == True:              
                            if B2 > 0:
                                ...
                                p4 = v(w,h,L-Z2/2+ZC2)
                            ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,L+abs(Z2+ZC2)/2)))
                            ProfileCut1 = ProfileExt.cut(self.SubtractiveCoin(p1,p2,p3,p4,H,A,k))
                            if B1 > 0:
                                p1 = v(w,h,-Z1/2)
                                p2 = v(w+W,h,Z1/2)
                                p3 = v(w+W,h,-Z1/2-ZC1)
                                ...
                                p4 = v(w,h,-Z1/2-ZC1)
                            ProfileExt = ProfileCut1.fuse(p.extrude(v(0,0,-ZC1/5)))    
                            ProfileCut2 = ProfileExt.cut(self.SubtractiveCoin(p1,p2,p3,p4,H,A,k))
                            obj.Shape  = ProfileCut2.removeSplitter()   
   ...
   def SubtractiveCoin(obj,p1,p2,p3,p4,H,A,k):
        FaceBevel = Part.Face(Part.makePolygon([p1,p2,p3,p4,p1]))
        coin1 = FaceBevel.extrude(v(0,k*H,0))
        coin2 = FaceBevel.extrude(v(0,-k*H,0))
        if A:
