...
class Profile:
   ...
   def execute(self, obj):
        ...
        if W == 0 : W = H
        k = 3 * round (max (H/W,W/H),2)                 # coeff pour depasser l'objet (rotation bevel)
        ...
        if self.type == "Square Hollow" or self.type == "Rectangular Hollow" or self.type == "Pipe":
            p1 = Part.Face(wire1)
            p2 = Part.Face(wire2)  
            p = p1.cut(p2)
        else:
                ...
                if B1 and not B2:
                    if obj.WidthCentered == False: 
                        if B1 < 0:
                            ...
                            p2 = v(W+w,h,Z1)
                            p3 = v(w,h,Z1)
                            ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,Z1)))
                            ProfileCut = ProfileExt.cut(self.SubtractiveCoin(p1,p2,p3,H,A,d,k))
                    ...
                    if obj.WidthCentered == True:
                        if B1>0:
                            ...
                            p2 = v(w+W,h,-Z1/2)
                            p3 = v(w+W,h,Z1/2)
                            ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,-Z1/2)))    
                        else:
                            p1 = v(w,h,-Z1/2)
                            p2 = v(w+W,h,Z1/2)
                            p3 = v(w,h,Z1/2)
                ...
                if B2 and not B1: 
                    if obj.WidthCentered == False: 
                        if B2 < 0:
                            ...
                            p2 = v(w,h,L-Z2)
                            p3 = v(W+w,h,L-Z2)
                            ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,L-Z2)))
                            ProfileCut = ProfileExt.cut(self.SubtractiveCoin(p1,p2,p3,H,A,d,k))
                            obj.Shape  = ProfileCut.removeSplitter()
                        else:
                            p1 = v(w,h,L)
                    ...
                    if obj.WidthCentered == True:
                        if B2<0:
                            ...
                            p2 = v(w+W,h,L-Z2/2)
                            p3 = v(w,h,L+Z2/2)
                            ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,L-Z2/2))) 
                        else:
                            p1 = v(w,h,L+Z2/2)
                            p2 = v(w+W,h,L+Z2/2)
                            p3 = v(w+W,h,L-Z2/2)
                ...
                if B1 and B2 :                  
                    if obj.WidthCentered == False: 
                        ...
                        if B2 < 0:
                            ...
                            p3 = v(W+w,h,L-Z2)
                            ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,L-Z2)))
                            Profile1 = ProfileExt.cut(self.SubtractiveCoin(p1,p2,p3,H,A,d,k))
                        else:
                            p1 = v(w,h,L)
                            p2 = v(W+w,h,L)
                            p3 = v(W+w,h,L-Z2)
                            Profile1  = ProfileFull.cut(self.SubtractiveCoin(p1,p2,p3,H,A,d,k))   
                        if B1 < 0:
                            p1 = v(w,h,0)
                            p2 = v(W+w,h,Z1)
                            p3 = v(w,h,Z1)
                            ProfileExt = Profile1.fuse(p.extrude(v(0,0,Z1)))
                            Profile2 = ProfileExt.cut(self.SubtractiveCoin(p1,p2,p3,H,A,d,k))
                        else:
                            p1 = v(w,h,0)
                            p2 = v(W+w,h,0)
                            p3 = v(W+w,h,Z1)
                            Profile2  = Profile1.cut(self.SubtractiveCoin(p1,p2,p3,H,A,d,k))     
                        obj.Shape  = Profile2.removeSplitter()    
                    ...
                    if obj.WidthCentered == True:              
                            ...
                            if B1>0:
                                ...
                                p2 = v(w+W,h,-Z1/2)
                                p3 = v(w+W,h,Z1/2)
                                ProfileExt = Profile1.fuse(p.extrude(v(0,0,-Z1/2)))    
                            else:
                                p1 = v(w,h,-Z1/2)
                                p2 = v(w+W,h,Z1/2)
                                p3 = v(w,h,Z1/2)
