...
class Box(QtGui.QDialog):
    ...
    def onCheckbox6(self):
        if self.BevelsCombined == False:
            self.BevelsCombined = True
        else:
            self.BevelsCombined = False     
   ...
   def execute(self, obj):
        ...
        if L:                                   
            ...
            if B1Y or B2Y or B1X or B2X or B1Z or B2Z:       # make the bevels:        
                ...
                # box.translate (v(-hc/2+w,-hc/2+h,L))
                # pr = v(-w,-h,L)
                # box.rotate(pr,v(0,1,0),B2Y)
                # box.rotate(pr,v(0,0,1),B2Z)
                # ProfileExt = ProfileTemp.fuse(box)
                
                # box = Part.makeBox(hc,hc,hc)
                # box.translate (v(-hc/2+w,-hc/2+h,-hc))
                # pr = v(-w,-h,0)
                # box.rotate(pr,v(0,1,0),B1Y)
                # box.rotate(pr,v(0,0,1),B1Z)
                # ProfileTemp = ProfileExt.fuse(box)
                
                
                
                ProfileExt = ProfileTemp.fuse(p.extrude(v(0,0,L+hc/4)))
                box = Part.makeBox(hc,hc,hc)
                box.translate (v(-hc/2+w,-hc/2+h,L))
                pr = v(0,0,L)
                box.rotate(pr,v(0,1,0),B2Y)
                if self.BevelCombined == True:  box.rotate(pr,v(0,0,1),B2Z)
                else: box.rotate(pr,v(1,0,0),B2X)
                ProfileCut = ProfileExt.cut(box)
                
                ProfileExt = ProfileCut.fuse(p.extrude(v(0,0,-hc/4)))
                box = Part.makeBox(hc,hc,hc)
                box.translate (v(-hc/2+w,-hc/2+h,-hc))
                pr = v(0,0,0)
                box.rotate(pr,v(0,1,0),B1Y)
                if self.BevelCombined == True:  box.rotate(pr,v(0,0,1),B1Z)
                else: box.rotate(pr,v(1,0,0),B1X)
                ProfileCut = ProfileExt.cut(box)
