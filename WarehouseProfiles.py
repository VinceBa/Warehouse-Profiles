...
class Profile:
   ...
   def execute(self, obj):
        ...
        if L:                                   
            ...
            if B1 and not B2:
                if B1 < 0 : 
                    ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,-W)))
                    ProfileCom = ProfileExt.common(self.SubtractiveCoin(p1,p2,p3,H,A,d,k))
                    ProfilePos = ProfileFull.fuse(ProfileCom)
                    obj.Shape = ProfilePos.removeSplitter()
                else:         
                    obj.Shape  = ProfileFull.cut(self.SubtractiveCoin(p1,p2,p3,H,A,d,k))
                        
            if B2 and not B1: 
                if B2 < 0: 
                    ProfileExt = ProfileFull.fuse(p.extrude(v(0,0,L+W)))
                    ProfileCom = ProfileExt.common(self.SubtractiveCoin(p1,p2,p3,H,A,d,k))
                    ProfilePos = ProfileFull.fuse(ProfileCom)
                    obj.Shape = ProfilePos.removeSplitter()
                else:         
            ...
            if B1 and B2 :  
                if B1 < 0: 
                    ...
                    ProfileCom = ProfileExt.common(self.SubtractiveCoin(p1,p2,p3,H,A,d,k))
                    ProfilePos = ProfileFull.fuse(ProfileCom)
                    Profiledemicut = ProfilePos.removeSplitter()
                else:         
                ...
                if B2 < 0: 
                    ProfileExt = Profiledemicut.fuse(p.extrude(v(0,0,L+W)))
                    ProfileCom = ProfileExt.common(self.SubtractiveCoin(p1,p2,p3,H,A,d,k))
                    ProfilePos = Profiledemicut.fuse(ProfileCom)
                    Profiledemicut2 = ProfilePos.removeSplitter()
                    ProfileTemp = Profiledemicut.fuse(Profiledemicut2)
                    obj.Shape = ProfileTemp.removeSplitter()
                else: 
